...
class CENOMamba2Mixer(nn.Module):
    ...
    def cuda_kernels_forward(
        ...
        # Ensure hidden_states is 3D before any operations
        ...
        # Fused calculations or step by step if no initialized cache is found
            ...
            # MODIFIED: Always use training path (conv1d isolated) for inference
    ...
    # fmt: off
    def torch_forward(self, input_states, cache_params: Optional[HybridMambaAttentionDynamicCache]=None, cache_position:Optional[torch.LongTensor]=None, attention_mask: Optional[torch.Tensor]=None, seq_idx: Optional[torch.Tensor]=None):
    ...
    def forward(
        ...
        if seq_idx is not None:
            raise RuntimeError(
                "CENO-P MSA scoring requires the Mamba CUDA kernels: the PyTorch fallback "
                "cannot preserve per-segment Mamba state resets. Install mamba-ssm and causal-conv1d."
            )
        if attention_mask is not None and attention_mask.shape[1] > 1 and attention_mask.shape[0] > 1:
            # tune out hidden states for pad tokens, see https://github.com/state-spaces/mamba/issues/66
            hidden_states = (hidden_states * attention_mask[:, :, None]).to(dtype)

        return self.torch_forward(hidden_states, cache_params, cache_position, attention_mask, seq_idx=seq_idx)
...
class CENOPForCausalLM(CENOPreTrainedModel, GenerationMixin):
